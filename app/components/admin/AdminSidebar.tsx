...
const navItems = [
  ...
  { label: "Merchants", href: "/admin/merchants", icon: IconUser },
  { label: "Payments", href: "/admin/payments", icon: IconTable },
  { label: "Providers", href: "/admin/providers", icon: IconLayers },
  { label: "Routing", href: "/admin/routing", icon: IconRoute },
  { label: "Callbacks", href: "/admin/callbacks", icon: IconSend },
  { label: "Reconciliation", href: "/admin/reconciliation", icon: IconSync },
  { label: "Logs", href: "/admin/logs", icon: IconFile },
];
      ...
      <nav className="admin-sidebar-scroll flex-1 overflow-y-auto overflow-x-hidden px-0 pb-6 pt-1">
          ...
          {navItems.map((item) => {
            ...
            return (
              <li key={item.href} className="relative">
                ...
                >
                  {active && (
                    <span className="absolute left-0 top-1 bottom-1 w-0.75 rounded-r bg-[#ff5e16]" />
                  )}
        ...
        {!collapsed && (
          <div className="mx-5 mt-6 rounded-lg border border-white/10 bg-white/5 px-3 py-3">
            <div className="mb-1 flex items-center gap-2 text-[11px] font-semibold uppercase tracking-wide text-white/45">
              <IconChart className="h-3.5 w-3.5" />
              Scope
            </div>
            <p className="text-[12px] leading-relaxed text-white/60">
              Platform admin: merchants, providers, routing, and ops logs.
            </p>
          </div>
        )}
      </nav>
