...
export default function MerchantWebhooksPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true);
      try {
        const data = await fetchMerchantCallbacks({
        ...
        if (cancelled) return;
        toast.error(
          err instanceof Error ? err.message : "Failed to load webhook logs.",
        );
      } finally {
  ...
  return (
    <div className="w-full space-y-5">
      <PageHeader
        title="Webhook Logs"
        description="Delivery attempts for payment status notifications sent to your webhook URL. Failed deliveries retry automatically for up to 5 attempts."
      />
      ...
      <Card>
        ...
        {loading ? (
          ...
          <EmptyState message="No webhook deliveries yet. They'll show up here once you configure a webhook URL in Settings and receive a payment." />
        ) : (
          <TableShell>
              ...
              <tbody>
                {items.map((row) => (
                  ...
                  >
                    ...
                    <td className="max-w-55 truncate px-5 py-3.5 text-[#6b7c93]">
                      {row.target_url}
                    </td>
